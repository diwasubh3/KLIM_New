...
IsActive = 1,
ParentFundId=5,
ParentFundCode='CLO-9',
CLOFileName='TradeDateCashCLO9{dateid}.csv',
DisplayText='9',
PortfolioName='York Warehouse (CLO-9)',
SortOrder=900,
BloombergCode='YCLO 2021-1'
WHERE FundId = 5


GO

INSERT INTO CLO.FundRestriction
(
    FundId,
    FundRestrictionTypeId,
    FieldId,
    OperatorId,
    RestrictionValue
)
SELECT FundId,FundRestrictionTypeId,FieldId,OperatorId,RestrictionValue FROM CLO.FundRestriction WHERE FundId = 10
GO

ALTER TABLE CLO.AggregatedPosition
ADD [CLO9NumExposure] [NUMERIC](32, 4) NULL,
...
SELECT [FieldGroupId]
      ...
      ,[HeaderCellClass]
      ,[CellClass]
      ,'<div class="ui-grid-cell-contents"  uib-tooltip="{{row.entity.clO9NumExposure| currency : '''' : 2}}" style="cursor:pointer;" tooltip-append-to-body="true"><span>{{row.entity.clO9Exposure}} </span></div>'
      ,[Hidden]
      ,[PinnedLeft]
      ,[IsSecurityOverride]
      ,[ShowInFilter]
...
ALTER VIEW [CLO].[vw_PivotedCostPrice]
AS
...
ALTER VIEW CLO.vw_PivotedPositionCountry
AS
...
SELECT 
		  ...
		  , CAST(AVG(CASE WHEN c.FundId in (4,10,2) THEN NULL else c.WARF end)AS NUMERIC(20,2))			WARF					
		  ...
		  , CAST(AVG(CASE WHEN c.FundId <> 10 THEN NULL else c.WARF end)AS NUMERIC(20,2))			[Fund-10 WARF]
		  , MAX(m.[Fund 10 MoodyRecovery]) [Fund-10 MoodyRecovery]
		  , CAST(AVG(CASE WHEN c.FundId <> 2 THEN NULL else c.WARF end)AS NUMERIC(20,2))			[Fund-2 WARF]
	...
	SET WARF = [CLO].SafeDivideby((ISNULL([Fund-4 WARF],0) + ISNULL([Fund-10 WARF],0)+ ISNULL([Fund-2 WARF],0)),
	((CASE WHEN ISNULL([Fund-4 WARF],0) <> 0 then 1 ELSE 0 END)+(CASE WHEN ISNULL([Fund-10 WARF],0) <> 0 then 1 ELSE 0 END)+(CASE WHEN ISNULL([Fund-2 WARF],0) <> 0 then 1 ELSE 0 END)))
	WHERE [PositionDateId] = CLO.GetPrevDayDateId() AND WARF IS NULL AND COALESCE([Fund-4 WARF],[Fund-10 WARF],[Fund-2 WARF]) IS NOT NULL

	UPDATE [CLO].[AggregatedPosition]
	SET [MoodyRecovery] = COALESCE([Fund-4 MoodyRecovery],[Fund-10 MoodyRecovery],[Fund-2 MoodyRecovery])
	WHERE [PositionDateId] = CLO.GetPrevDayDateId() AND [MoodyRecovery] IS NULL AND COALESCE([Fund-4 MoodyRecovery],[Fund-10 MoodyRecovery],[Fund-2 MoodyRecovery]) IS NOT NULL
