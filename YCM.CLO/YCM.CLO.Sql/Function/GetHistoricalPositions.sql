CREATE FUNCTION [CLO].[GetHistoricalPositions](@date DATETIME)
...
AS	(
		SELECT PositionId, FundId, SecurityId, DateId, MarketValue, Exposure, PctExposure, PxPrice, IsCovLite, CountryId
			, CreatedOn, CreatedBy, LastUpdatedOn, LastUpdatedBy, IsStale, CapitalizedInterestOrig
		FROM CLO.Position WITH (NOLOCK)
		WHERE DateId = dbo.GetDateIdFromDate(@date) OR PositionId IS NULL
	)
SELECT p.PositionId, p.DateId PositionDateId
	...
	, ISNULL(p.CapitalizedInterestOrig, 0) CapitalizedInterestOrig, p.PxPrice
	...
	, cs.ScoreDescription
	, a.Sponsor
FROM CLO.GetHistoricalSecurityFund(@date) s
	...
	LEFT JOIN CLO.vw_AllMarketData m WITH (NOLOCK)
	ON s.SecurityId = m.SecurityId AND s.FundId = m.FundId AND m.DateId = dbo.GetDateIdFromDate(@date)
	LEFT JOIN CLO.vw_AllMarketData pm WITH (NOLOCK)
	ON s.SecurityId = pm.SecurityId AND s.FundId = pm.FundId AND pm.DateId = DATEADD(DAY,-1, @date)
	LEFT JOIN CLO.vw_AllAnalystResearch a WITH (NOLOCK)
	ON a.IssuerId = s.IssuerId AND a.AsOfDate = @date
	LEFT JOIN CLO.vw_AllCalculation c WITH (NOLOCK)
	ON s.SecurityId = c.SecurityId AND s.FundId = c.FundId AND c.DateId = dbo.GetDateIdFromDate(@date)
