...
namespace YCM.CLO.DataAccess
{
	public class Repository : IRepository
	{
		...
		private void AddColumnIfMissing(CustomView view, string userName, short fieldId)
		{
			...
			{
				FieldId = fieldId, IsPinned = false, SortOrder = 9999999, CreatedBy = userName, LastUpdatedBy = userName
				, CreatedOn = DateTime.Now, LastUpdatedOn = DateTime.Now, ViewId = view.ViewId
			};
        ...
        public bool AddMatrixPoint(MatrixPoint matrixData,string user)
        {
            using (CLOContext context = new CLOContext())
            {
                    context.MatrixPoints.Add(new MatrixPoint() { FundId = matrixData.FundId,
                    CreatedBy = user, CreatedOn = DateTime.Now,
