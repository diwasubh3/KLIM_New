...
(function ($) {
    ...
    $.fn.YorkNetNav.BindMenu = function (data) {
        ...
        //if (!yorkNavigationMenu.loadedMenuFromServer) {
        //    getFavorites();
        //} else {
    ...
    function renderNavbar(html, options) {
        ...
        //html[html.length] = '<div >'; //navbar
        //html[html.length] = '<div class="navbar-inner">'; //navbar-inner
        //html[html.length] = '<div class="container">'; //container
        //html[html.length] = '<ul class="nav"><li><a href="javascript:;" id="menu-button" title="Toggle Menu"><i class="icon-th-large icon-border-hidden"></i></a></li></ul>';
        if (options.pageTitle != null) {
            console.log('options.pageTitle' + options.pageTitle)
            options.pageTitle = options.pageTitle.replace("> (no title)", "");
        }

        html[html.length] = '<a id="york-navigationmenu-brand" class="brand" data-container="body" data-toggle="tooltip" data-placement="bottom" data-original-title="' + options.brudCrumbTooltip + '" href_temp="' + options.pageUri + '">' + options.pageTitle + '</a>';
        html[html.length] = '<div class="nav-collapse">'; //nav-collapse
        html[html.length] = '<ul class="nav pull-right">'; //nav pull-right
        ...
        html[html.length] = '<li><a href="#" onclick="window.open(\'http://yorkhda/hdaccess\',\'HelpWindow\')" id="help-button" title="Help"><i class="icon-question-sign"></i></a></li>';
        ...
        //html[html.length] = '</div>'; //navbar-inner
        //html[html.length] = '</div>'; //navbar
    }
    ...
    function renderMenuContent(html, options, menu) {
        ...
        html[html.length] = '<i class="' + menu.Icon + '"></i><span>' + menu.Title + '</span>';
        if (menu.Children.length > 0)
            html[html.length] = '<b class="caret"></b>';
        html[html.length] = '</a>';
        if (menu.Children.length > 0)
